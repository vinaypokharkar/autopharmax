...
- `backend/`: FastAPI backend for serving the model and data.
- `frontend/`: Streamlit UI for predictions.
- `dataset/`: Raw and merged data files.
- `trained_models/`: Trained models and scaler.
- `training/`: Jupyter notebooks for data preparation and model training.
- `docker-compose.yml`: Docker Compose file for running the application.
- `Dockerfile`: Dockerfile for the backend and frontend services.
...
1) Open `training/expirements/mergedatasets.ipynb`
...
Use `training/expirements/after.ipynb` to:
