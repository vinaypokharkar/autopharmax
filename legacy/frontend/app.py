...
def chat_with_dr_ai(query: str, context: dict):
    try:
        # Context includes cell_line, drug_name, predicted values, and shap_values
        payload = {"query": query, "context": context}
        response = requests.post(f"{API_BASE_URL}/chat", json=payload, timeout=60)
        response.raise_for_status()
        return response.json().get("response", "No response from AI.")
    except Exception as e:
        return f"Dr. AI is currently unavailable: {str(e)}"
    ...
    if st.button("🔬 Predict IC50"):
        with st.spinner("🔄 Making prediction..."):
            ...
            if result:
                ...
                st.session_state.last_selection = {"cell_line": selected_cell_line, "drug_name": selected_drug}
                ...
                if result.get('shap_values'):
                    st.markdown("---")
                    st.markdown("<h3 style='text-align: center;'>🧠 Model Insights (Explainable AI)</h3>", unsafe_allow_html=True)
                    st.write("Top genomic markers driving this prediction:")
                    
                    shap_df = pd.DataFrame(list(result['shap_values'].items()), columns=['Feature', 'SHAP Value'])
                    shap_df['Abs_SHAP'] = shap_df['SHAP Value'].abs()
                    top_shap = shap_df.sort_values(by='Abs_SHAP', ascending=False).head(10)
                    
                    st.bar_chart(top_shap.set_index('Feature')['SHAP Value'], use_container_width=True)
                    st.info("💡 **Positive values** (to the right) increase LN(IC50), making the drug *less effective*. **Negative values** (to the left) decrease LN(IC50), making the drug *more effective*.")

                # ============================================
                # DR. AI CHATBOX
                # ============================================
                st.markdown("---")
                st.markdown("<h3 style='text-align: center;'>👨‍⚕️ Ask Dr. AI (Clinical Insights)</h3>", unsafe_allow_html=True)
                
                # Combine info for context
                full_context = {**result, **st.session_state.last_selection}
                
                if 'chat_history' not in st.session_state:
                    st.session_state.chat_history = []
                
                # Input for help
                user_query = st.text_input("Ask a clinical question about this result (e.g., 'Why is this drug effective for this cell line?')", key="user_query")
                
                if st.button("💬 Ask Dr. AI"):
                    if user_query:
                        with st.spinner("Dr. AI is analyzing the genomic markers..."):
                            ai_response = chat_with_dr_ai(user_query, full_context)
                            st.session_state.chat_history.append({"query": user_query, "response": ai_response})
                    else:
